class PagesController < ApplicationController
  ...
  def search
    if params[:search].blank?
      ...
      @results = Cliente.all.where("lower(nombre) LIKE :search OR lower(apellido) LIKE :search OR lower(domicilio) LIKE :search  OR lower(telefono) LIKE :search OR lower(CUIT) LIKE :search OR lower(correo) LIKE :search OR lower(cliente_tipo) LIKE :search OR (celular) LIKE :search OR (celular) LIKE :search OR (cuit) LIKE :search", search: "%#{@parameter}%")
    end
  end
